...
namespace NUnit.Framework
{
    ...
    public static partial class AssertEx
    {
        ...
        {
#if WINRT
            if (typeof(T) != typeof(string) && typeof(IEnumerable).GetTypeInfo().IsAssignableFrom(typeof(T).GetType().GetTypeInfo()))
#else
        ...
        private class ReflectAccessor<T>
        {
            ...
            {
#if WINRT
                var field = typeof(T).GetTypeInfo().GetDeclaredField( name );
#else
...
#if WINRT
                var prop = typeof(T).GetTypeInfo().GetDeclaredProperty( name );
#else
